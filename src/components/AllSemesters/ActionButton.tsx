...
import { updateResults, removeResults } from '@/redux/slice';


const ActionButton = ({id, rowIndex, rowData}: EditGPA) => {
    ...
    const deleteResult = () => {
        ...
        dispatch(removeResults({id, index: rowIndex}))
    };
    ...
    const handleSubmit = (values: GpaAttributesForm) => {
            ...
            dispatch(updateResults({id, index: rowIndex, results: validResults}))
            setShowDialog(false);
        };
